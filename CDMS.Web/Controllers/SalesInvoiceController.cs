...
namespace CDMS.Web.Controllers
{
    ...
    {        
        private readonly ISalesInvoiceComplexService _SalesInvoiceComplexService;        
        private readonly IGlobalService _GlobalService;

        public SalesInvoiceController(
               IGlobalService globalService,
               ISalesInvoiceComplexService SalesInvoiceComplexService
               )
        {                
            this._SalesInvoiceComplexService = SalesInvoiceComplexService;
            ...
            string orderby, string sort, int page = 1)
        {
            ...
            //   sql += " && (SalesInvoiceDate >=(@0))";

            //if (!string.IsNullOrEmpty(finish))
            //    sql += " && (SalesInvoiceDate <=(@1))";

            //if (!string.IsNullOrEmpty(company))
            //    sql += " && (CompanySupplier.Contains(@2))";

            var query = 
                this._SalesInvoiceComplexService.GetAll().Where(sql, obj.ToArray());
        ...
        {
            var info = new SalesInvoiceComplex()
            {

#if DEBUG
                Invoice = new SalesInvoiceViewModel()
                {                                      
                    InvoiceID = "",
                ...
                ChildList = new List<SalesInvoiceDetailViewModel>() {
        ...
        public ActionResult Edit(string id)
        {
            try
            {
                ...
                #region 取資料
                var query = this._SalesInvoiceComplexService.Get(id);
                #endregion
        ...
        public ActionResult RemoveChild(long id)
        {
            ...
            try
            {
                ...
                if (id != 0)
                {
                    this._SalesInvoiceComplexService.RemoveChild(id);
                }
                #endregion
