    ...
    public class WorkplaceController : BaseController
    {
        ...
        public ActionResult _List(string txt, string orderby, string sort, int page = 1)
        {
            ...
            if (!string.IsNullOrEmpty(txt))
                Sql += " && (CX_Workplace.Contains(@0) || CX_Workplace_Remarks.Contains(@0))";

            var query = this._workplaceService.GetAll().Where(Sql, obj.ToArray());
        ...
        public ActionResult Edit(int? id)
        {
            try
            {
                ...
                #region 取資料
                var query = this._workplaceService.GetById((int)id);
                #endregion
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult Edit(Workplace model)
        {
            ResultModel result = new ResultModel();
            try
            {
                ...
                #region Service資料庫
                this._workplaceService.Update(model);
                #endregion
        ...
        [ValidateAntiForgeryToken]
        public ActionResult DeleteConfirmed(Workplace model)
        {
            ResultModel result = new ResultModel();
            try
            {
                #region Service資料庫
                this._workplaceService.Delete(model);
                #endregion
