...
{
    public class OverTypeController : BaseController
    {
        ...
        public ActionResult _List(string txt, string orderby, string sort, int page = 1)
        {
            ...
            if (!string.IsNullOrEmpty(txt))
                Sql += " && (CX_OverType.Contains(@0))";//|| CX_Country_Remarks.Contains(@0)

            var query = this._overtypeService.GetAll().Where(Sql, obj.ToArray());
        ...
        public ActionResult Edit(int? id)
        {
            try
            {
                ...
                #region 取資料
                var query = this._overtypeService.GetById((int)id);
                #endregion
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult Edit(OverType model)
        {
            ResultModel result = new ResultModel();
            try
            {
                ...
                #region Service資料庫
                this._overtypeService.Update(model);
                #endregion
